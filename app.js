...
async function loadData(){
  try{
    for(const f of files) DATA[f] = await fetch(`data/${f}.json`).then(r=>{ if(!r.ok) throw new Error(`Falha ao carregar ${f}`); return r.json(); });
    ...
    enrichExactLists();
    init();
    toast('Dados carregados. Ranking recalculado sem total manual.');
...
function flagImg(code){
  if(!code) return '';
  // V3: usar emoji como bandeira principal. Isso evita falha por CDN/bloqueio externo no GitHub Pages.
  // A imagem remota fica apenas como reforço visual, e o emoji permanece sempre visível.
  const iso = TEAM_ISO[code];
  const label = esc(teamName(code));
  const emoji = `<span class="flag-emoji" role="img" aria-label="Bandeira de ${label}">${flagEmoji(code)}</span>`;
  const img = iso ? `<img class="flag-img optional-flag-img" src="https://flagcdn.com/w20/${iso}.png" alt="" loading="lazy" onerror="this.remove()"/>` : '';
  return `<span class="flag-stack">${emoji}${img}</span>`;
}
  ...
  for(const r of CALC.ranking){
    ...
    d.exact_rows = [
      ...(d.group_predictions||[])
        ...
        .map(x=>{ const g=gamesById[x.game_id]||{}; return {kind:'Grupo', game_id:x.game_id, points:x.points, prediction:x.prediction, result:g.score||x.result, reason:'Placar exato na fase de grupos', home:g.home, away:g.away}; }),
      ...(d.knockout_predictions||[])
        .filter(x=>Number(x.points_placar||0)>0)
        .map(x=>{ const g=gamesById[x.game_id]||{}; return {kind:phaseLabel(phaseKey(x.game_id)), game_id:x.game_id, points:x.points_placar, prediction:x.prediction_score, result:g.score||x.result_in_pdf, reason:'Placar exato no mata-mata com confronto e classificado corretos', home:g.home, away:g.away}; })
    ];
  }
}
function auditInternal(){
  ...
  for(const r of CALC.ranking){
    ...
    const exactCount=(d.exact_rows||[]).length;
    if(exactCount!==r.cravadas) issues.push(`${r.display_name}: cravadas ${r.cravadas} diferem da lista ${exactCount}`);
...
function renderPalpites(){
  ...
  const draw = ()=>{ const q = $('palpiteSearch').value.toLowerCase().trim(), teamCode = $('palpiteTeam').value, phase = $('palpitePhase').value, game = $('palpiteGame').value.trim().replace('#',''), status = $('palpiteStatus').value; const filtered = allRows.filter(x => (!q || x.participant.toLowerCase().includes(q)) && (teamCode==='Todas' || x.teams.includes(teamCode)) && (phase==='Todas' || x.phase===phase) && (!game || String(x.game_id)===game) && (status==='Todos' || String(x.status||'').toLowerCase().includes(status.toLowerCase()))); $('palpitesCount').textContent = `${fmt.format(filtered.length)} palpites encontrados`; $('palpitesBody').innerHTML = filtered.map(x=>`<tr><td><button class="participant-btn" data-open-entry="${x.entry_id}">${esc(x.participant)}</button></td><td>${phaseLabel(x.phase)}</td><td>#${x.game_id}<br><small>${x.official}</small></td><td>${x.palpite}</td><td>${x.result}</td><td>${badge(x.status)}</td><td class="num total">${x.points}</td></tr>`).join('') || `<tr><td colspan="7" class="empty-state">Nenhum palpite encontrado.</td></tr>`; bindOpeners(); };
...
function detailTabContent(r,d,tab){
  ...
  if(tab==='resumo') return `<div class="rules-grid compact"><article><strong>Classificados corretos</strong><p>${r.classificados_acertos} seleções · ${r.classificados} pontos</p></article><article><strong>Mata sem classificados</strong><p>${r.ko_confronto + r.ko_placar} pontos</p></article><article><strong>Pontos por avanço</strong><p>${r.ko_avanco} pontos</p></article><article><strong>Cravadas</strong><p>${r.cravadas_grupo} em grupos · ${r.cravadas_mata_mata} no mata-mata</p></article></div><h4>Atalho de conferência</h4><p class="muted-text">Abra as abas “Jogos que pontuaram”, “Jogos sem ponto” e “Cravadas” para ver a origem de cada ponto.</p>`;
...
function renderStats(){ const mostExact = [...CALC.ranking].sort((a,b)=>b.cravadas-a.cravadas || a.posicao-b.posicao)[0], played=DATA.resultados.filter(isPlayed).length, pending=DATA.resultados.length-played; $('statsCards').innerHTML = `<section class="panel stats-card"><h4>👥 Participantes</h4><b>${DATA.estatisticas.entries_valid}</b><p>${DATA.estatisticas.entries_invalid} apostas fora da pontuação</p></section><section class="panel stats-card"><h4>⚽ Jogos</h4><b>${played}</b><p>${pending} pendentes</p></section><section class="panel stats-card"><h4>🎯 Rei/Rainha das cravadas</h4><b>${esc(mostExact.display_name)}</b><p>${mostExact.cravadas} placares exatos conferíveis</p></section>`; renderPhaseStats(); renderTeamPhaseStats(); renderFinalsStats(); renderScorerStats(); renderExactStats(); renderHardEasyGames(); renderEvolution(); }
...
function renderFinalsStats(){ const positions=[['campeao','Campeão'],['vice','Vice'],['terceiro','3º lugar'],['quarto','4º lugar']]; $('finalsStats').innerHTML = positions.map(([key,label])=>`<section class="stat-tile"><h4>${label}</h4>${barsFromMap(finalPositionMap(key))}</section>`).join(''); }
