...
function exactRankingSection(title, rows, mode){
  return `<section class="stat-tile"><h4>${title}</h4>${rows.map(r=>{
    ...
    const label = mode==='grupo' ? 'cravada(s) de grupos' : mode==='mata' ? 'cravada(s) de mata-mata' : 'cravada(s) totais';
    return `<details><summary><span>${esc(r.display_name)}</span><b>${n} ${label}</b></summary><div class="exact-mini-proof"><span>Grupos: <b>${cravadasGrupo(r)}</b></span><span>Mata-mata: <b>${cravadasMata(r)}</b> / ${r.ko_placar||0} pts de bônus</span><span>Total estatístico: <b>${cravadasTotal(r)}</b></span></div>${exactHTML(detailsByEntry[r.entry_id], mode)}</details>`;
  }).join('')}</section>`;
}
...
function participantDetailHTML(id, modal){
  ...
  return `<div class="summary-grid cravada-summary-grid"><div class="summary-card"><span>Posição</span><b>${r.posicao}º</b></div><div class="summary-card"><span>Total</span><b>${r.total}</b></div><div class="summary-card"><span>Jogos de grupo</span><b>${r.grupos_jogos}</b></div><div class="summary-card"><span>Classificados</span><b>${r.classificados}</b></div><div class="summary-card"><span>Mata-mata</span><b>${r.mata_mata}</b></div><div class="summary-card cravada-group"><span>🎯 Cravadas de grupos</span><b>${cravadasGrupo(r)}</b><small>placar exato · 5 pts</small></div><div class="summary-card cravada-ko"><span>⭐ Cravadas de mata-mata</span><b>${cravadasMata(r)}</b><small>${r.ko_placar||0} pts de bônus</small></div><div class="summary-card cravada-total"><span>✅ Total estatístico</span><b>${cravadasTotal(r)}</b><small>grupos + mata-mata</small></div></div><div class="sum-proof"><strong>Prova da soma:</strong> ${r.grupos_jogos} jogos de grupo + ${r.classificados} classificados + ${r.ko_confronto} confronto + ${r.ko_avanco} avanço + ${r.ko_placar} bônus de placar no mata-mata + ${r.bonus||0} bônus = <b>${r.total}</b><br><strong>Cravadas para estatísticas:</strong> <b>${cravadasGrupo(r)}</b> de grupos (valem 5 pts no cálculo) + <b>${cravadasMata(r)}</b> de mata-mata (valem ${r.ko_placar||0} pts de bônus) = <b>${cravadasTotal(r)}</b>. A pontuação oficial não usa o total estatístico para reordenar o ranking.</div><div class="phase-mini-grid">${koPhases.map(ph=>`<div><span>${phaseLabel(ph)}</span><b>${b[ph]?.total||0}</b><small>${b[ph]?.jogos||0} jogos · ${b[ph]?.avanco||0} avanço · ${b[ph]?.bonus||0} bônus</small></div>`).join('')}</div><div class="detail-actions"><button class="btn tiny" data-copy-summary="${id}">Copiar resumo</button>${modal?'<button class="btn tiny ghost-dark" data-go-detail="conferencia">Abrir na conferência</button>':''}</div><div class="detail-tabs">${tabs.map(([key,label])=>`<button class="${currentDetailTab===key?'active':''}" data-detail-tab="${key}">${label}</button>`).join('')}</div><div class="detailTabContent">${detailTabContent(r,d,currentDetailTab)}</div>`;
}
...
function detailTabContent(r,d,tab){
  ...
  if(tab==='cravadas') return exactSplitHTML(d);
...
function exactRowsHTML(rows, emptyText){
  const html=(rows||[]).map(x=>{ const g=gamesById[x.game_id]; const isKo=String(x.kind||'').toLowerCase().includes('mata'); return `<div class="game-row exact-row ${isKo?'ko-exact':'group-exact'}"><div class="match-no">#${x.game_id}</div><div><div>${team(g?.home,true)} <b class="x">x</b> ${team(g?.away,true)}</div><small><b>Palpite:</b> ${esc(x.prediction||'—')} · <b>Real:</b> ${esc(x.result||score(g))} · <b>${isKo?'Mata-mata':'Grupo'}</b> · ${x.points} ponto(s)</small><small>${isKo?'⭐ Cravada de mata-mata · +3 pontos de bônus, sem virar cravada de 5':'🎯 Cravada de grupos · placar exato valendo 5 pontos'}</small></div><div class="score-badge target">${isKo?'⭐':'🎯'}</div></div>`; }).join('');
  return html || `<div class="empty-state">${emptyText}</div>`;
}
function exactHTML(d, mode='total'){
  if(mode==='grupo') return exactRowsHTML(d?.exact_group_rows||[], 'Nenhuma cravada de grupos.');
  if(mode==='mata') return exactRowsHTML(d?.ko_exact_rows||[], 'Nenhuma cravada de mata-mata.');
  return exactRowsHTML(d?.exact_rows||[], 'Nenhuma cravada pontuada.');
}
function exactSplitHTML(d){
  const groupRows = d?.exact_group_rows || [];
  const koRows = d?.ko_exact_rows || [];
  const koPts = koRows.reduce((s,x)=>s+Number(x.points||0),0);
  return `<div class="cravada-split-head"><article><span>Cravadas de grupos</span><b>${groupRows.length}</b><small>placar exato · 5 pts cada</small></article><article><span>Cravadas de mata-mata</span><b>${koRows.length}</b><small>${koPts} pts de bônus · 3 pts cada</small></article><article><span>Total estatístico</span><b>${groupRows.length + koRows.length}</b><small>não altera regra de pontos</small></article></div><section class="cravada-section group"><h4>🎯 Cravadas de grupos</h4>${exactRowsHTML(groupRows, 'Nenhuma cravada de grupos.')}</section><section class="cravada-section mata"><h4>⭐ Cravadas de mata-mata / bônus de placar</h4><p class="muted-text">No mata-mata, o placar exato só entra quando confronto e classificado também estão corretos. Cada uma vale 3 pontos de bônus.</p>${exactRowsHTML(koRows, 'Nenhuma cravada de mata-mata.')}</section>`;
}
